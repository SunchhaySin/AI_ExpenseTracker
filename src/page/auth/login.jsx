...
export default function LoginPage() {
  ...
  const { setLoggedInUser, windowWidth } = UseAppContext();
    ...
    <div>
      ...
      >
        <div className="flex-1 flex items-center gap-2 self-center text-(--text-orange)">
          ...
          <p className={`${windowWidth > 1275 ? "text-xl" : windowWidth > 750 ? "text-lg" : "text-md" }`}>Account Login</p>
        </div>
        <div className="flex-1 flex flex-col gap-4 items-center justify-center">
          ...
          <button
            className={`flex gap-2 justify-center items-center bg-(--bg2) p-2 ${windowWidth > 750 ? "w-100" : windowWidth > 389 ? "w-80" : "w-65"} rounded-lg text-(--text-d) text-lg mt-5 hover:bg-(--text-orange) ${windowWidth > 750 ? "text-md" : "text-sm"}`}
            type="submit"
            form="loginForm"
          >
            {isLoading ? (
              ...
              <p className={`${windowWidth > 750 ? "text-md" : "text-sm"}`}>Login</p>
            )}
            {!isLoading && (
              <svg
                xmlns="http://www.w3.org/2000/svg"
